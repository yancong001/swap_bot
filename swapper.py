...
from lib.swap_bot_exceptions import *
import lib.abi_lib
from lib import style
from lib.utils import json_file_load, is_valid_evm_address
from data import pool_kyberswap

POOL_CONTRACTS = pool_kyberswap.POOL_CONTRACTS
...
class Swapper:
    ...
    def quote(self, input_token: (str, ChecksumAddress), output_token: (str, ChecksumAddress), qty_: int = 0,
        ...
        """
        if self.debug_mode:
            self._print.debug('Function `quote` called with arguments: `%s %s %s %s %s %s %s`' % (
            input_token, output_token, qty_, symbol, decimals, output_token, out_decimals))
    ...
    def swap(self, input_token: str, output_token: str, float_qty: (float, int) = 0, raw_qty: int = 0,
             recipient: str = None,
        ...
        """
        if self.debug_mode:
            self._print.debug('Function `swap` called with arguments: `%s %s %s %s %s %s %s %s`' % (
            input_token, output_token, float_qty, raw_qty, recipient, no_prompt, fee_on_transfer, _quote_only))
        ...
        input_symbol, input_decimals, input_token, output_token, out_symbol, out_decimals = self.verify(input_token,
                                                                                                        output_token)
        if self.debug_mode:
            self._print.debug('Verify: %s %s %s %s %s %s' % (
            input_symbol, input_decimals, input_token, output_token, out_symbol, out_decimals))
        _qty = 0
        if raw_qty > 0:
            _qty = raw_qty
        if float_qty > 0:
    ...
    def price2tick(self, price, base_token_decimal=6, quote_token_decimal=18, spacing=60):
        # =========================================================================
    ...
    def add_position(self,
        ...
        # if is_valid_evm_address(pool):
        #     pass
        # else:
        #     self._print.warning('Wrong contract address.')
        #     return ''
        input_symbol, base_token_decimal, base_token, quote_token, out_symbol, quote_token_decimal = self.verify(
            base_token.upper(),
            quote_token.upper())

        amount_0_qty = amount_0 * 10 ** base_token_decimal
        amount_1_qty = amount_1 * 10 ** quote_token_decimal
        tickLower = self.price2tick(upperPrice, base_token_decimal=base_token_decimal,
                                    quote_token_decimal=quote_token_decimal)
        tickUpper = self.price2tick(lowerPrice, base_token_decimal=base_token_decimal,
                                    quote_token_decimal=quote_token_decimal)
        print(self.router_contract_addr)
        return self.uniswap.mint_liquidity(self.router_contract_addr, amount_0_qty, amount_1_qty, tickLower, tickUpper)
...
if __name__ == '__main__':
    ...
    s = style.PrettyText()
    args = argparse.ArgumentParser(usage='Swap_bot Usage. See docs for details.')
    args.add_argument('-N', '--network', dest='network_name', default='ethereum', choices=['ethereum', 'polygon'],
                      help='The network to connect to.')
    args.add_argument('-uv', '--uniswap_version', type=int, default=3, choices=[2, 3], help='Uniswap version.')
    ...
    args.add_argument('-b', '--backend', type=str, choices=['uniswap', 'kyberswap'], default='kyberswap',
    ...
    cmd_add_position = subparsers.add_parser('add_position', help='add pool liquidity .')
    # cmd_add_position.add_argument('-p', '--pool', dest='pool', type=str, default=POOL_CONTRACTS['FRAX_USDC'],
    #                               help='Contract address or known token symbol.')
    cmd_add_position.add_argument('-i', '--input', dest='base_token', type=str, default='FRAX',
                                  help='Contract address or known token symbol.')
    cmd_add_position.add_argument('-o', '--output', dest='quote_token', type=str,default='USDC',
                                  help='Contract address or known token symbol.')
    cmd_add_position.add_argument('-a0', '--amount_0', dest='amount_0', type=float, default=1.0, help='Float quantity.')
    cmd_add_position.add_argument('-a1', '--amount_1', dest='amount_1', type=float, default=1.0, help='Float quantity.')
    cmd_add_position.add_argument('-l', '--lowerPrice', dest='lowerPrice', type=float, default=0.1,
                                  help='Float quantity.')
    cmd_add_position.add_argument('-u', '--upperPrice', dest='upperPrice', type=float, default=2.0,
                                  help='Float quantity.')
    cmd_add_position.add_argument('-n', '--no_prompt', dest='no_prompt', action='store_true',
                                  help='Do not prompt to confirm quote.')

    qty = 0
    private_key = '80b9a82f11cdd1813554299e82c885800869a726bf500fb5dd476abd3bf9660c'
    address = '0x4f558e5C97203b1CC92d4C963b434D69072Aad29'
    quote_only = False
    ...
    if args.command == 'swap' or args.command == 'quote':
        ...
        for x, argument in enumerate(['input_token', 'output_token']):
            if getattr(args, argument) is None and x == 0:
                ...
                # setattr(args, 'input_token', 'FRAX')
            if getattr(args, argument) is None and x > 0:
                setattr(args, 'output_token', 'USDC')
        s.normal(
            f'Params: IN: {args.input_token}, OUT: {args.output_token}, QTY: {args.quantity}, RAW: {args.raw_quantity}')

        txid = uni.swap(input_token=args.input_token, output_token=args.output_token, float_qty=args.quantity,
                        raw_qty=args.raw_quantity, recipient=args.recipient_address,
    ...
    if args.command == 'add_position':
        # if not hasattr(args, 'pool'):
            # setattr(args, 'pool', POOL_CONTRACTS['FRAX_USDC'])
        if not hasattr(args, 'base_token'):
            setattr(args, 'base_token', 'FRAX')
        if not hasattr(args, 'quote_token'):
            setattr(args, 'base_token', 'USDC')
        if not hasattr(args, 'amount_0'):
            setattr(args, 'amount_0', 1.0)
        if not hasattr(args, 'amount_1'):
            setattr(args, 'amount_1', 1.0)
        if not hasattr(args, 'lowerPrice'):
            setattr(args, 'lowerPrice', 0.0)
        if not hasattr(args, 'upperPrice'):
            setattr(args, 'upperPrice', 2.0)

        s.normal(
            f'Params: base_token: {args.base_token}, quote_token: {args.quote_token}, amount_0: {args.amount_0}, amount_1: {args.amount_1}'
            f' lowerPrice: {args.lowerPrice}, upperPrice: {args.upperPrice}')

        txid = uni.add_position(base_token=args.base_token, quote_token=args.quote_token,
                        amount_0=args.amount_0, amount_1=args.amount_1,
                        lowerPrice=args.lowerPrice, upperPrice=args.upperPrice)
        if txid == '':
            exit(0)
        else:
            print(txid)
